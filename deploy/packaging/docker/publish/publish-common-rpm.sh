...
if command -v aws >/dev/null 2>&1 ; then
	if [[ ! -z "$GEOWAVE_VERSION_URL" ]]; then
		...
		aws s3 rm --recursive --quiet ${WORKSPACE}/target/site/ s3://geowave/${GEOWAVE_VERSION_URL}/docs/
		...
		aws s3 rm --recursive --quiet ${WORKSPACE}/target/site/ s3://geowave/${GEOWAVE_VERSION_URL}/scripts/
