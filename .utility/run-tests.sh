...
if [ "$IT_ONLY" == "true" ]; then
  ...
  mvn -q verify -D -Dtest=SkipUnitTests -DfailIfNoTests=false -Dformatter.skip=true -Dfindbugs.skip=true -Daccumulo.version=$ACCUMULO_VERSION -Daccumulo.api=$ACCUMULO_API -Dhbase.version=$HBASE_VERSION -Dhadoop.version=$HADOOP_VERSION -Dgeotools.version=$GEOTOOLS_VERSION -Dgeoserver.version=$GEOSERVER_VERSION -P $PLATFORM_VERSION
else
